...
func (rt *reflectTest) TestIsEmptyValue_validValues() {
	...
	rt.False(util.IsEmptyValue(reflect.ValueOf(map[string]any{"a": true})))
...
func (rt *reflectTest) TestGetColumnMap_withStructbuilderTags() {
	type TestStruct struct {
		Str    string `builder:"skipinsert,skipupdate"`
		Int    int64  `builder:"skipinsert"`
		Bool   bool   `builder:"skipupdate"`
		Empty  bool   `builder:"defaultifempty"`
		Valuer *sql.NullString
	}
	var ts TestStruct
	cm, err := util.GetColumnMap(&ts)
...
func (rt *reflectTest) TestGetColumnMap_withStructWithTagAndbuilderTag() {
	type TestStruct struct {
		Str    string          `db:"s" builder:"skipinsert,skipupdate"`
		Int    int64           `db:"i" builder:"skipinsert"`
		Bool   bool            `db:"b" builder:"skipupdate"`
		Valuer *sql.NullString `db:"v"`
	}
	var ts TestStruct
	cm, err := util.GetColumnMap(&ts)
...
func (rt *reflectTest) TestGetColumnMap_withNonStruct() {
	...
	rt.EqualError(err, "builder: cannot scan into this type: int64")
}
