...
func (dsgs *deleteSQLGeneratorSuite) TestGenerate_withUnsupportedFragment() {
	...
	dsgs.assertCases(
		...
		deleteTestCase{clause: dc, err: `builder: unsupported DELETE SQL fragment InsertBeingSQLFragment`},
		deleteTestCase{clause: dc, err: `builder: unsupported DELETE SQL fragment InsertBeingSQLFragment`, isPrepared: true},
	)
}
...
func (dsgs *deleteSQLGeneratorSuite) TestGenerate_withErroredBuilder() {
	...
	d.Generate(b, dc)
	dsgs.assertErrorSQL(b, "builder: expected error")
...
func (dsgs *deleteSQLGeneratorSuite) TestGenerate_withWhere() {
	...
	dsgs.assertCases(
		...
		deleteTestCase{clause: dc, sql: `DELETE FROM "test" WHERE "a"=?`, isPrepared: true, args: []any{
			int64(1),
		}},
	)
}
...
func (dsgs *deleteSQLGeneratorSuite) TestGenerate_withLimit() {
	...
	dsgs.assertCases(
		...
		deleteTestCase{clause: dc, sql: `DELETE FROM "test" LIMIT ?`, isPrepared: true, args: []any{int64(1)}},
	)
...
func (dsgs *deleteSQLGeneratorSuite) TestGenerate_withReturning() {
	...
	expectedErr := `builder: dialect does not support RETURNING clause [dialect=test]`
	dsgs.assertCases(
		sqlgen.NewDeleteSQLGenerator("test", opts),
