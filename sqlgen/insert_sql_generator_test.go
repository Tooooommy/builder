...
func (igs *insertSQLGeneratorSuite) TestGenerate_UnsupportedFragment() {
	...
	d.Generate(b, ic)
	igs.assertErrorSQL(b, `builder: unsupported INSERT SQL fragment UpdateBeginSQLFragment`)
}
...
func (igs *insertSQLGeneratorSuite) TestGenerate_nilValues() {
	...
	igs.assertCases(
		...
		insertTestCase{clause: ic, sql: `INSERT INTO "test" ("a") VALUES (?)`, isPrepared: true, args: []any{nil}},
	)
}
...
func (igs *insertSQLGeneratorSuite) TestGenerate_colsAndVals() {
	...
	igs.assertCases(
		...
		insertTestCase{clause: ic, sql: `INSERT INTO "test" {"a"; "b"} values {#; #}; {#; #}; {#; #}`, isPrepared: true, args: []any{
			"a1", "b1", "a2", "b2", "a3", "b3",
		}},

		insertTestCase{clause: bic, err: `builder: rows with different value length expected 1 got 2`},
		insertTestCase{clause: bic, err: `builder: rows with different value length expected 1 got 2`, isPrepared: true},
	)
}
...
func (igs *insertSQLGeneratorSuite) TestGenerate_withNoInto() {
	...
	expectedErr := "builder: no source found when generating insert sql"
	igs.assertCases(
		sqlgen.NewInsertSQLGenerator("test", opts),
...
func (igs *insertSQLGeneratorSuite) TestGenerate_withRowsAppendableExpression() {
	ic := exp.NewInsertClauses().
		SetInto(exp.NewIdentifierExpression("", "test", "")).
		SetRows([]any{newTestAppendableExpression(`select * from "other"`, emptyArgs, nil, nil)})
...
func (igs *insertSQLGeneratorSuite) TestGenerate_onConflict() {
	...
	igs.assertCases(
		...
		insertTestCase{clause: icDuBad, err: "builder: unsupported update interface type bool"},
		insertTestCase{clause: icDuBad, err: "builder: unsupported update interface type bool", isPrepared: true},
	)
	...
	expectedErr := "builder: dialect does not support upsert with where clause [dialect=test]"
	igs.assertCases(
		sqlgen.NewInsertSQLGenerator("test", opts),
...
func (igs *insertSQLGeneratorSuite) TestGenerate_withCommonTables() {
	...
	expectedErr := "builder: dialect does not support CTE WITH clause [dialect=test]"
	igs.assertCases(
		sqlgen.NewInsertSQLGenerator("test", opts),
	...
	expectedErr = "builder: dialect does not support CTE WITH RECURSIVE clause [dialect=test]"
	igs.assertCases(
		sqlgen.NewInsertSQLGenerator("test", opts),
		insertTestCase{
			clause: icCte1,
...
func (igs *insertSQLGeneratorSuite) TestGenerate_withReturning() {
	...
	igs.assertCases(
		...
		insertTestCase{clause: ic, sql: `INSERT INTO "test" ("a", "b") VALUES (?, ?) RETURNING "a", "b"`, isPrepared: true, args: []any{
			"a1", "b1",
		}},
	)
}
