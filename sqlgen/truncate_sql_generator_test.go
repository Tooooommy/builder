...
func (tsgs *truncateSQLGeneratorSuite) TestGenerate() {
	...
	expectedNoSourceErr := "builder: no source found when generating truncate sql"
	tsgs.assertCases(
...
func (tsgs *truncateSQLGeneratorSuite) TestGenerate_UnsupportedFragment() {
	...
	expectedErr := "builder: unsupported TRUNCATE SQL fragment UpdateBeginSQLFragment"
	tsgs.assertCases(
		sqlgen.NewTruncateSQLGenerator("test", opts),
...
func (tsgs *truncateSQLGeneratorSuite) TestGenerate_WithErroredBuilder() {
	...
	tsgs.assertErrorSQL(b, `builder: expected error`)
}
