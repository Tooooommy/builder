...
func (ssgs *selectSQLGeneratorSuite) TestGenerate_UnsupportedFragment() {
	...
	sc := exp.NewSelectClauses().SetFrom(exp.NewColumnListExpression("test"))
	expectedErr := "builder: unsupported SELECT SQL fragment InsertBeingSQLFragment"
	ssgs.assertCases(
		sqlgen.NewSelectSQLGenerator("test", opts),
...
func (ssgs *selectSQLGeneratorSuite) TestGenerate_WithErroredBuilder() {
	...
	d.Generate(b, c)
	ssgs.assertErrorSQL(b, `builder: test error`)
}
...
func (ssgs *selectSQLGeneratorSuite) TestGenerate_withDistinct() {
	...
	expectedErr := "builder: dialect does not support DISTINCT ON clause [dialect=test]"
	ssgs.assertCases(
		sqlgen.NewSelectSQLGenerator("test", opts),
		selectTestCase{clause: sc, sql: `SELECT DISTINCT *`},
		selectTestCase{clause: sc, sql: `SELECT DISTINCT *`, isPrepared: true},
...
func (ssgs *selectSQLGeneratorSuite) TestGenerate_withJoin() {
	...
	expectedRjError := "builder: dialect does not support RightJoinType"
	expectedJoinCondError := "builder: join condition required for conditioned join LeftJoinType"
	ssgs.assertCases(
		sqlgen.NewSelectSQLGenerator("test", opts),
...
func (ssgs *selectSQLGeneratorSuite) TestGenerate_withWhere() {
	...
	ssgs.assertCases(
		...
		selectTestCase{clause: scWhere1, sql: `SELECT * FROM "test" where ("a" = ?)`, isPrepared: true, args: []any{"b"}},
...
func (ssgs *selectSQLGeneratorSuite) TestGenerate_withHaving() {
	...
	ssgs.assertCases(
		...
		selectTestCase{clause: scHaving1, sql: `SELECT * FROM "test" having ("a" = ?)`, isPrepared: true, args: []any{"b"}},
...
func (ssgs *selectSQLGeneratorSuite) TestGenerate_withLimit() {
	...
	ssgs.assertCases(
		...
		selectTestCase{clause: sc, sql: `SELECT * FROM "test" LIMIT ?`, isPrepared: true, args: []any{int64(10)}},
	)
}
...
func (ssgs *selectSQLGeneratorSuite) TestGenerate_withOffset() {
	...
	ssgs.assertCases(
		...
		selectTestCase{clause: sc, sql: `SELECT * FROM "test" OFFSET ?`, isPrepared: true, args: []any{int64(10)}},
	)
}
...
func (ssgs *selectSQLGeneratorSuite) TestToSelectSQL_withFor() {
	...
	scFnSlOf := sc.SetLock(exp.NewLock(exp.ForNolock, exp.SkipLocked, builder.T("my_table")))
	...
	scFsSlOf := sc.SetLock(exp.NewLock(exp.ForShare, exp.SkipLocked, builder.T("my_table")))
	scFsSlOfMulti := sc.SetLock(exp.NewLock(exp.ForShare, exp.SkipLocked, builder.T("my_table"), builder.T("table2")))
	...
	ssgs.assertCases(
		...
		selectTestCase{clause: scFnSlOf, sql: `SELECT * FROM "test"`, isPrepared: true, args: []any{}},
