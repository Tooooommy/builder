...
func (csgs *commonSQLGeneratorSuite) TestReturningSQL() {
	...
	csgs.assertCases(
		...
		commonSQLTestCase{gen: returningGen(csgs2), err: `builder: dialect does not support RETURNING clause [dialect=test]`},
		commonSQLTestCase{gen: returningGen(csgs2), err: `builder: dialect does not support RETURNING clause [dialect=test]`},
	)
}
...
func (csgs *commonSQLGeneratorSuite) TestWhereSQL() {
	...
	csgs.assertCases(
		...
		commonSQLTestCase{gen: whereAndGen(csg, w), sql: ` WHERE ("a" = ?)`, isPrepared: true, args: []any{"b"}},

		commonSQLTestCase{gen: whereAndGen(csg, w, w2), sql: ` WHERE (("a" = 'b') AND ("b" = 'c'))`},
		commonSQLTestCase{gen: whereAndGen(csg, w, w2), sql: ` WHERE (("a" = ?) AND ("b" = ?))`, isPrepared: true, args: []any{"b", "c"}},
		...
		commonSQLTestCase{gen: whereOrGen(csg, w), sql: ` WHERE ("a" = ?)`, isPrepared: true, args: []any{"b"}},

		commonSQLTestCase{gen: whereOrGen(csg, w, w2), sql: ` WHERE (("a" = 'b') OR ("b" = 'c'))`},
		commonSQLTestCase{gen: whereOrGen(csg, w, w2), sql: ` WHERE (("a" = ?) OR ("b" = ?))`, isPrepared: true, args: []any{"b", "c"}},
		...
		commonSQLTestCase{gen: whereAndGen(csgWhereFrag, w), sql: ` where ("a" = ?)`, isPrepared: true, args: []any{"b"}},
		...
		commonSQLTestCase{gen: whereOrGen(csgWhereFrag, w), sql: ` where ("a" = ?)`, isPrepared: true, args: []any{"b"}},
...
func (csgs *commonSQLGeneratorSuite) TestLimitSQL() {
	limitGen := func(csgs sqlgen.CommonSQLGenerator, l any) func(sb.SQLBuilder) {
		return func(sb sb.SQLBuilder) {
			csgs.LimitSQL(sb, l)
		}
	}
	...
	csgs.assertCases(
		...
		commonSQLTestCase{gen: limitGen(csg, l), sql: ` LIMIT ?`, isPrepared: true, args: []any{l}},
		...
		commonSQLTestCase{gen: limitGen(csgCustom, l), sql: ` limit ?`, isPrepared: true, args: []any{l}},
...
func (csgs *commonSQLGeneratorSuite) TestUpdateExpressionSQL() {
	...
	csgs.assertCases(
		...
		commonSQLTestCase{gen: updateGen(csg, ue), sql: `"col"=?`, isPrepared: true, args: []any{"a"}},

		commonSQLTestCase{gen: updateGen(csg, ue, ue2), sql: `"col"='a',"col2"='b'`},
		commonSQLTestCase{gen: updateGen(csg, ue, ue2), sql: `"col"=?,"col2"=?`, isPrepared: true, args: []any{"a", "b"}},
	)
}
