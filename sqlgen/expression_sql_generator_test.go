...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_Invalid() {
	...
	esgs.assertCases(
		...
		expressionTestCase{val: b, sql: "?", isPrepared: true, args: []any{nil}},
	)
}
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_UnsupportedType() {
	...
	esgs.assertCases(
		sqlgen.NewExpressionSQLGenerator("test", sqlgen.DefaultDialectOptions()),
		expressionTestCase{val: strct{}, err: "builder_encode_error: Unable to encode value {}"},
		expressionTestCase{val: strct{}, err: "builder_encode_error: Unable to encode value {}", isPrepared: true},
	)
}
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_FloatTypes() {
	...
	esgs.assertCases(
		...
		expressionTestCase{val: float32(10.01), sql: "?", isPrepared: true, args: []any{float64(float32(10.01))}},

		expressionTestCase{val: float64(10.01), sql: "10.01"},
		expressionTestCase{val: float64(10.01), sql: "?", isPrepared: true, args: []any{float64(10.01)}},

		expressionTestCase{val: &float, sql: "0"},
		expressionTestCase{val: &float, sql: "?", isPrepared: true, args: []any{float}},
	)
}
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_IntTypes() {
	...
	for _, i := range ints {
		esgs.assertCases(
			...
			expressionTestCase{val: i, sql: "?", isPrepared: true, args: []any{int64(10)}},
		)
	}
	esgs.assertCases(
		sqlgen.NewExpressionSQLGenerator("test", sqlgen.DefaultDialectOptions()),
		expressionTestCase{val: &i, sql: "0"},
		expressionTestCase{val: &i, sql: "?", isPrepared: true, args: []any{i}},
	)
}
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_StringTypes() {
	...
	esgs.assertCases(
		...
		expressionTestCase{val: "Hello", sql: "?", isPrepared: true, args: []any{"Hello"}},

		expressionTestCase{val: "Hello'", sql: "'Hello'''"},
		expressionTestCase{val: "Hello'", sql: "?", isPrepared: true, args: []any{"Hello'"}},

		expressionTestCase{val: &str, sql: "''"},
		expressionTestCase{val: &str, sql: "?", isPrepared: true, args: []any{str}},
	)
}
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_BytesTypes() {
	esgs.assertCases(
		...
		expressionTestCase{val: []byte("Hello"), sql: "?", isPrepared: true, args: []any{[]byte("Hello")}},
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_BoolTypes() {
	...
	esgs.assertCases(
		...
		expressionTestCase{val: true, sql: "?", isPrepared: true, args: []any{true}},

		expressionTestCase{val: false, sql: "FALSE"},
		expressionTestCase{val: false, sql: "?", isPrepared: true, args: []any{false}},

		expressionTestCase{val: &bl, sql: "FALSE"},
		expressionTestCase{val: &bl, sql: "?", isPrepared: true, args: []any{bl}},
	)
}
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_TimeTypes() {
	...
	esgs.assertCases(
		...
		expressionTestCase{val: ts, sql: "?", isPrepared: true, args: []any{ts}},
	...
	esgs.assertCases(
		...
		expressionTestCase{val: nt, sql: "?", isPrepared: true, args: []any{nil}},
	)
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_NilTypes() {
	esgs.assertCases(
		...
		expressionTestCase{val: nil, sql: "?", isPrepared: true, args: []any{nil}},
	)
}
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_Valuer() {
	...
	esgs.assertCases(
		...
		expressionTestCase{
			val: datasetValuerType{int: 10}, sql: "?", isPrepared: true, args: []any{[]byte("Hello World 10")},
		},

		expressionTestCase{val: datasetValuerType{err: err}, err: "builder: valuer error"},
		expressionTestCase{
			val: datasetValuerType{err: err}, isPrepared: true, err: "builder: valuer error",
		},
		expressionTestCase{
			val: val, sql: "NULL",
		},
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_Slice() {
	esgs.assertCases(
		...
		expressionTestCase{
			val: []string{"a", "b", "c"}, sql: "(?, ?, ?)", isPrepared: true, args: []any{"a", "b", "c"},
		},

		expressionTestCase{val: []byte{'a', 'b', 'c'}, sql: `'abc'`},
		expressionTestCase{
			val: []byte{'a', 'b', 'c'}, sql: "?", isPrepared: true, args: []any{[]byte{'a', 'b', 'c'}},
		},
	)
}
...
func (esgs *expressionSQLGeneratorSuite) TestGenerateUnsupportedExpression() {
	errMsg := "builder: unsupported expression type sqlgen_test.unknownExpression"
	esgs.assertCases(
		sqlgen.NewExpressionSQLGenerator("test", sqlgen.DefaultDialectOptions()),
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_AppendableExpression() {
	...
	a := newTestAppendableExpression(`select * from "a"`, []any{}, nil, nil)
	aliasedA := newTestAppendableExpression(`select * from "a"`, []any{}, nil, ti)
	argsA := newTestAppendableExpression(`select * from "a" where x=?`, []any{true}, nil, ti)
	...
	esgs.assertCases(
		...
		expressionTestCase{val: ae, err: "builder: expected error"},
		expressionTestCase{val: ae, err: "builder: expected error", isPrepared: true},

		expressionTestCase{val: argsA, sql: `(select * from "a" where x=?) AS "b"`, args: []any{true}},
		expressionTestCase{val: argsA, sql: `(select * from "a" where x=?) AS "b"`, isPrepared: true, args: []any{true}},
	)
}
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_ExpressionList() {
	...
	esgs.assertCases(
		...
		expressionTestCase{
			val: andEl, sql: `(("a" = ?) AND ("c" != ?))`, isPrepared: true, args: []any{"b", int64(1)},
		},

		expressionTestCase{val: orEl, sql: `(("a" = 'b') OR ("c" != 1))`},
		expressionTestCase{
			val: orEl, sql: `(("a" = ?) OR ("c" != ?))`, isPrepared: true, args: []any{"b", int64(1)},
		},
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_BooleanExpressionAliased() {
	...
	esgs.assertCases(
		...
		expressionTestCase{val: ident.Eq(1).As("b"), sql: `("a" = ?) AS "b"`,
			isPrepared: true, args: []any{int64(1)}},
	)
}
func (esgs *expressionSQLGeneratorSuite) TestGenerate_BooleanExpression() {
	...
	esgs.assertCases(
		...
		expressionTestCase{val: ident.Eq(1), sql: `("a" = ?)`, isPrepared: true, args: []any{int64(1)}},
		...
		expressionTestCase{val: ident.Eq([]int64{1, 2, 3}), sql: `("a" IN (?, ?, ?))`, isPrepared: true, args: []any{
		...
		expressionTestCase{val: ident.Neq(1), sql: `("a" != ?)`, isPrepared: true, args: []any{int64(1)}},
		...
		expressionTestCase{val: ident.Neq([]int64{1, 2, 3}), sql: `("a" NOT IN (?, ?, ?))`, isPrepared: true, args: []any{
			int64(1), int64(2), int64(3),
		}},
		...
		expressionTestCase{val: ident.Gt(1), sql: `("a" > ?)`, isPrepared: true, args: []any{int64(1)}},

		expressionTestCase{val: ident.Gte(1), sql: `("a" >= 1)`},
		expressionTestCase{val: ident.Gte(1), sql: `("a" >= ?)`, isPrepared: true, args: []any{int64(1)}},

		expressionTestCase{val: ident.Lt(1), sql: `("a" < 1)`},
		expressionTestCase{val: ident.Lt(1), sql: `("a" < ?)`, isPrepared: true, args: []any{int64(1)}},

		expressionTestCase{val: ident.Lte(1), sql: `("a" <= 1)`},
		expressionTestCase{val: ident.Lte(1), sql: `("a" <= ?)`, isPrepared: true, args: []any{int64(1)}},

		expressionTestCase{val: ident.In([]int64{1, 2, 3}), sql: `("a" IN (1, 2, 3))`},
		expressionTestCase{val: ident.In([]int64{1, 2, 3}), sql: `("a" IN (?, ?, ?))`, isPrepared: true, args: []any{
			int64(1), int64(2), int64(3),
		}},
		...
		expressionTestCase{val: ident.NotIn([]int64{1, 2, 3}), sql: `("a" NOT IN (?, ?, ?))`, isPrepared: true, args: []any{
			int64(1), int64(2), int64(3),
		}},
		...
		expressionTestCase{val: ident.Like("a%"), sql: `("a" LIKE ?)`, isPrepared: true, args: []any{"a%"}},

		expressionTestCase{val: ident.Like(re), sql: `("a" ~ '[ab]')`},
		expressionTestCase{val: ident.Like(re), sql: `("a" ~ ?)`, isPrepared: true, args: []any{"[ab]"}},

		expressionTestCase{val: ident.ILike("a%"), sql: `("a" ILIKE 'a%')`},
		expressionTestCase{val: ident.ILike("a%"), sql: `("a" ILIKE ?)`, isPrepared: true, args: []any{"a%"}},

		expressionTestCase{val: ident.ILike(re), sql: `("a" ~* '[ab]')`},
		expressionTestCase{val: ident.ILike(re), sql: `("a" ~* ?)`, isPrepared: true, args: []any{"[ab]"}},

		expressionTestCase{val: ident.NotLike("a%"), sql: `("a" NOT LIKE 'a%')`},
		expressionTestCase{val: ident.NotLike("a%"), sql: `("a" NOT LIKE ?)`, isPrepared: true, args: []any{"a%"}},

		expressionTestCase{val: ident.NotLike(re), sql: `("a" !~ '[ab]')`},
		expressionTestCase{val: ident.NotLike(re), sql: `("a" !~ ?)`, isPrepared: true, args: []any{"[ab]"}},

		expressionTestCase{val: ident.NotILike("a%"), sql: `("a" NOT ILIKE 'a%')`},
		expressionTestCase{val: ident.NotILike("a%"), sql: `("a" NOT ILIKE ?)`, isPrepared: true, args: []any{"a%"}},

		expressionTestCase{val: ident.NotILike(re), sql: `("a" !~* '[ab]')`},
		expressionTestCase{val: ident.NotILike(re), sql: `("a" !~* ?)`, isPrepared: true, args: []any{"[ab]"}},
	)
	...
	esgs.assertCases(
		sqlgen.NewExpressionSQLGenerator("test", opts),
		expressionTestCase{val: ident.Eq(1), err: "builder: boolean operator 'eq' not supported"},
		expressionTestCase{val: ident.Neq(1), err: "builder: boolean operator 'neq' not supported"},
		expressionTestCase{val: ident.Is(true), err: "builder: boolean operator 'is' not supported"},
		expressionTestCase{val: ident.IsNot(true), err: "builder: boolean operator 'isnot' not supported"},
		expressionTestCase{val: ident.Gt(1), err: "builder: boolean operator 'gt' not supported"},
		expressionTestCase{val: ident.Gte(1), err: "builder: boolean operator 'gte' not supported"},
		expressionTestCase{val: ident.Lt(1), err: "builder: boolean operator 'lt' not supported"},
		expressionTestCase{val: ident.Lte(1), err: "builder: boolean operator 'lte' not supported"},
		expressionTestCase{val: ident.In([]int64{1, 2, 3}), err: "builder: boolean operator 'in' not supported"},
		expressionTestCase{val: ident.NotIn([]int64{1, 2, 3}), err: "builder: boolean operator 'notin' not supported"},
		expressionTestCase{val: ident.Like("a%"), err: "builder: boolean operator 'like' not supported"},
		expressionTestCase{val: ident.Like(re), err: "builder: boolean operator 'regexplike' not supported"},
		expressionTestCase{val: ident.ILike("a%"), err: "builder: boolean operator 'ilike' not supported"},
		expressionTestCase{val: ident.ILike(re), err: "builder: boolean operator 'regexpilike' not supported"},
		expressionTestCase{val: ident.NotLike("a%"), err: "builder: boolean operator 'notlike' not supported"},
		expressionTestCase{val: ident.NotLike(re), err: "builder: boolean operator 'regexpnotlike' not supported"},
		expressionTestCase{val: ident.NotILike("a%"), err: "builder: boolean operator 'notilike' not supported"},
		expressionTestCase{val: ident.NotILike(re), err: "builder: boolean operator 'regexpnotilike' not supported"},
	)
}
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_BitwiseExpression() {
	...
	esgs.assertCases(
		...
		expressionTestCase{val: ident.BitwiseAnd(1), sql: `("a" & ?)`, isPrepared: true, args: []any{int64(1)}},

		expressionTestCase{val: ident.BitwiseOr(1), sql: `("a" | 1)`},
		expressionTestCase{val: ident.BitwiseOr(1), sql: `("a" | ?)`, isPrepared: true, args: []any{int64(1)}},

		expressionTestCase{val: ident.BitwiseXor(1), sql: `("a" # 1)`},
		expressionTestCase{val: ident.BitwiseXor(1), sql: `("a" # ?)`, isPrepared: true, args: []any{int64(1)}},

		expressionTestCase{val: ident.BitwiseLeftShift(1), sql: `("a" << 1)`},
		expressionTestCase{val: ident.BitwiseLeftShift(1), sql: `("a" << ?)`, isPrepared: true, args: []any{int64(1)}},

		expressionTestCase{val: ident.BitwiseRightShift(1), sql: `("a" >> 1)`},
		expressionTestCase{val: ident.BitwiseRightShift(1), sql: `("a" >> ?)`, isPrepared: true, args: []any{int64(1)}},
	)
	...
	esgs.assertCases(
		sqlgen.NewExpressionSQLGenerator("test", opts),
		expressionTestCase{val: ident.BitwiseInversion(), err: "builder: bitwise operator 'Inversion' not supported"},
		expressionTestCase{val: ident.BitwiseAnd(1), err: "builder: bitwise operator 'AND' not supported"},
		expressionTestCase{val: ident.BitwiseOr(1), err: "builder: bitwise operator 'OR' not supported"},
		expressionTestCase{val: ident.BitwiseXor(1), err: "builder: bitwise operator 'XOR' not supported"},
		expressionTestCase{val: ident.BitwiseLeftShift(1), err: "builder: bitwise operator 'Left Shift' not supported"},
		expressionTestCase{val: ident.BitwiseRightShift(1), err: "builder: bitwise operator 'Right Shift' not supported"},
	)
}
func (esgs *expressionSQLGeneratorSuite) TestGenerate_RangeExpression() {
	...
	esgs.assertCases(
		...
		expressionTestCase{val: betweenNum, sql: `("a" BETWEEN ? AND ?)`, isPrepared: true, args: []any{
			int64(1),
			int64(2),
		}},

		expressionTestCase{val: notBetweenNum, sql: `("a" NOT BETWEEN 1 AND 2)`},
		expressionTestCase{val: notBetweenNum, sql: `("a" NOT BETWEEN ? AND ?)`, isPrepared: true, args: []any{
			int64(1),
			int64(2),
		}},

		expressionTestCase{val: betweenStr, sql: `("a" BETWEEN 'aaa' AND 'zzz')`},
		expressionTestCase{val: betweenStr, sql: `("a" BETWEEN ? AND ?)`, isPrepared: true, args: []any{
			"aaa",
			"zzz",
		}},

		expressionTestCase{val: notBetweenStr, sql: `("a" NOT BETWEEN 'aaa' AND 'zzz')`},
		expressionTestCase{val: notBetweenStr, sql: `("a" NOT BETWEEN ? AND ?)`, isPrepared: true, args: []any{
			"aaa",
			"zzz",
		}},
	)
	...
	esgs.assertCases(
		sqlgen.NewExpressionSQLGenerator("test", opts),
		expressionTestCase{val: betweenNum, err: "builder: range operator between not supported"},
		expressionTestCase{val: betweenNum, err: "builder: range operator between not supported"},

		expressionTestCase{val: notBetweenNum, err: "builder: range operator not between not supported"},
		expressionTestCase{val: notBetweenNum, err: "builder: range operator not between not supported"},

		expressionTestCase{val: betweenStr, err: "builder: range operator between not supported"},
		expressionTestCase{val: betweenStr, err: "builder: range operator between not supported"},

		expressionTestCase{val: notBetweenStr, err: "builder: range operator not between not supported"},
		expressionTestCase{val: notBetweenStr, err: "builder: range operator not between not supported"},
	)
}
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_UpdateExpression() {
	...
	esgs.assertCases(
		...
		expressionTestCase{val: ue, sql: `"a"=?`, isPrepared: true, args: []any{int64(1)}},
	)
}
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_SQLFunctionExpression() {
	...
	esgs.assertCases(
		...
		expressionTestCase{val: coalesce, sql: `COALESCE("a", ?)`, isPrepared: true, args: []any{"a"}},
	)
}
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_CommonTableExpressionSlice() {
	...
	esgs.assertCases(
		sqlgen.NewExpressionSQLGenerator("test", opts),
		expressionTestCase{val: cteNoArgs, err: "builder: dialect does not support CTE WITH clause [dialect=test]"},
		expressionTestCase{val: cteNoArgs, err: "builder: dialect does not support CTE WITH clause [dialect=test]", isPrepared: true},

		expressionTestCase{val: cteArgs, err: "builder: dialect does not support CTE WITH clause [dialect=test]"},
		expressionTestCase{val: cteArgs, err: "builder: dialect does not support CTE WITH clause [dialect=test]", isPrepared: true},

		expressionTestCase{val: cteRecursiveNoArgs, err: "builder: dialect does not support CTE WITH clause [dialect=test]"},
		expressionTestCase{val: cteRecursiveNoArgs, err: "builder: dialect does not support CTE WITH clause [dialect=test]", isPrepared: true},

		expressionTestCase{val: cteRecursiveArgs, err: "builder: dialect does not support CTE WITH clause [dialect=test]"},
		expressionTestCase{val: cteRecursiveArgs, err: "builder: dialect does not support CTE WITH clause [dialect=test]", isPrepared: true},
	)
	opts = sqlgen.DefaultDialectOptions()
	...
	esgs.assertCases(
		...
		expressionTestCase{
			...
			err: "builder: dialect does not support CTE WITH RECURSIVE clause [dialect=test]",
		},
		expressionTestCase{
			val:        cteRecursiveNoArgs,
			err:        "builder: dialect does not support CTE WITH RECURSIVE clause [dialect=test]",
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_IdentifierExpression() {
	...
	esgs.assertCases(
		...
		expressionTestCase{
			...
			err: `builder: a empty identifier was encountered, please specify a "schema", "table" or "column"`,
		},
		expressionTestCase{
		...
		expressionTestCase{
			...
			err: `builder: unexpected col type must be string or LiteralExpression received bool`,
		},
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_LateralExpression() {
	...
	esgs.assertCases(
		sqlgen.NewExpressionSQLGenerator("test", do),
		expressionTestCase{val: lateralExp, err: "builder: dialect does not support lateral expressions [dialect=test]"},
		expressionTestCase{val: lateralExp, err: "builder: dialect does not support lateral expressions [dialect=test]", isPrepared: true},
	)
}
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_CaseExpression() {
	...
	esgs.assertCases(
		...
		expressionTestCase{
			...
			isPrepared: true,
			args:       []any{int64(1), "one", int64(2), "two", "three"},
		},
		...
		expressionTestCase{
			...
			err: "builder: when conditions not found for case statement",
		},
	)
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_ExpressionMap() {
	esgs.assertCases(
		...
		expressionTestCase{val: exp.Ex{"a": 1}, sql: `("a" = ?)`, isPrepared: true, args: []any{int64(1)}},
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_ExpressionMapWithABadOp() {
	esgs.assertCases(
		...
		expressionTestCase{
			...
			err: "builder: unsupported expression type badOp",
		},
		expressionTestCase{
			val:        exp.Ex{"a": exp.Op{"badOp": true}},
			isPrepared: true,
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_ExpressionMapWithNeqOp() {
	esgs.assertCases(
		...
		expressionTestCase{val: exp.Ex{"a": exp.Op{"neq": 1}}, sql: `("a" != ?)`, isPrepared: true, args: []any{
			int64(1),
		}},
	)
}
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_ExpressionMapWithGtOp() {
	esgs.assertCases(
		...
		expressionTestCase{val: exp.Ex{"a": exp.Op{"gt": 1}}, sql: `("a" > ?)`, isPrepared: true, args: []any{
			int64(1),
		}},
	)
}
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_ExpressionMapWithGteOp() {
	esgs.assertCases(
		...
		expressionTestCase{val: exp.Ex{"a": exp.Op{"gte": 1}}, sql: `("a" >= ?)`, isPrepared: true, args: []any{
			int64(1),
		}},
	)
}
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_ExpressionMapWithLtOp() {
	esgs.assertCases(
		...
		expressionTestCase{val: exp.Ex{"a": exp.Op{"lt": 1}}, sql: `("a" < ?)`, isPrepared: true, args: []any{
			int64(1),
		}},
	)
}
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_ExpressionMapWithLteOp() {
	esgs.assertCases(
		...
		expressionTestCase{val: exp.Ex{"a": exp.Op{"lte": 1}}, sql: `("a" <= ?)`, isPrepared: true, args: []any{
			int64(1),
		}},
	)
}
