...
func (usgs *updateSQLGeneratorSuite) TestGenerate_unsupportedFragment() {
	...
	expectedErr := "builder: unsupported UPDATE SQL fragment InsertBeingSQLFragment"
	usgs.assertCases(
...
func (usgs *updateSQLGeneratorSuite) TestGenerate_withBadUpdateValues() {
	...
	expectedErr := "builder: unsupported update interface type bool"
	usgs.assertCases(
		sqlgen.NewUpdateSQLGenerator("test", sqlgen.DefaultDialectOptions()),
		updateTestCase{clause: uc, err: expectedErr},
		updateTestCase{clause: uc, err: expectedErr, isPrepared: true},
...
func (usgs *updateSQLGeneratorSuite) TestGenerate_withFrom() {
	...
	usgs.assertCases(
		...
		updateTestCase{clause: uc, sql: `UPDATE "test" SET "foo"=? FROM "other_test"`, isPrepared: true, args: []any{"bar"}},

		updateTestCase{clause: ucNullSet, sql: `UPDATE "test" SET "foo"=NULL FROM "other_test"`},
		updateTestCase{clause: ucNullSet, sql: `UPDATE "test" SET "foo"=? FROM "other_test"`, isPrepared: true, args: []any{nil}},
	)
	...
	usgs.assertCases(
		...
		updateTestCase{clause: uc, sql: `UPDATE "test","other_test" SET "foo"=?`, isPrepared: true, args: []any{"bar"}},

		updateTestCase{clause: ucNullSet, sql: `UPDATE "test","other_test" SET "foo"=NULL`},
		updateTestCase{clause: ucNullSet, sql: `UPDATE "test","other_test" SET "foo"=?`, isPrepared: true, args: []any{nil}},
	)

	opts = sqlgen.DefaultDialectOptions()
	opts.SupportsMultipleUpdateTables = false
	expectedErr := "builder: test dialect does not support multiple tables in UPDATE"
	usgs.assertCases(
		sqlgen.NewUpdateSQLGenerator("test", opts),
		updateTestCase{clause: uc, err: expectedErr},
		updateTestCase{clause: uc, err: expectedErr, isPrepared: true},
...
func (usgs *updateSQLGeneratorSuite) TestGenerate_withUpdateExpression() {
	...
	usgs.assertCases(
		...
		updateTestCase{clause: ucRecord, sql: `UPDATE "test" set "a"=?,"b"=?`, isPrepared: true, args: []any{"b", "c"}},

		updateTestCase{clause: ucRecordNullVal, sql: `UPDATE "test" set "a"='b',"b"=NULL`},
		updateTestCase{clause: ucRecordNullVal, sql: `UPDATE "test" set "a"=?,"b"=?`, isPrepared: true, args: []any{"b", nil}},

		updateTestCase{clause: ucRecordBoolVals, sql: `UPDATE "test" set "a"=TRUE,"b"=FALSE`},
		updateTestCase{clause: ucRecordBoolVals, sql: `UPDATE "test" set "a"=?,"b"=?`, isPrepared: true, args: []any{true, false}},
...
func (usgs *updateSQLGeneratorSuite) TestGenerate_withOrder() {
	...
	usgs.assertCases(
		...
		updateTestCase{clause: uc, sql: `UPDATE "test" SET "a"=?,"b"=?`, isPrepared: true, args: []any{"b", "c"}},
	)
}
...
func (usgs *updateSQLGeneratorSuite) TestGenerate_withLimit() {
	...
	usgs.assertCases(
		...
		updateTestCase{clause: uc, sql: `UPDATE "test" SET "a"=?,"b"=? LIMIT ?`, isPrepared: true, args: []any{"b", "c", int64(10)}},
	)
