...
func (tds *truncateDatasetSuite) TestGetClauses() {
	ds := builder.Truncate("test")
	ce := exp.NewTruncateClauses().SetTable(exp.NewColumnListExpression(builder.I("test")))
	tds.Equal(ce, ds.GetClauses())
}

func (tds *truncateDatasetSuite) TestTable() {
	bd := builder.Truncate("test")
	tds.assertCases(
		truncateTestCase{
			ds: bd.Table("test2"),
...
func (tds *truncateDatasetSuite) TestNoCascade() {
	bd := builder.Truncate("test").Cascade()
	tds.assertCases(
		truncateTestCase{
			ds: bd.NoCascade(),
			clauses: exp.NewTruncateClauses().
...
func (tds *truncateDatasetSuite) TestNoRestrict() {
	bd := builder.Truncate("test").Restrict()
	tds.assertCases(
		truncateTestCase{
			ds: bd.NoRestrict(),
			clauses: exp.NewTruncateClauses().
...
func (tds *truncateDatasetSuite) TestToSQL() {
	md := new(mocks.SQLDialect)
	ds := builder.Truncate("test").SetDialect(md)
	c := ds.GetClauses()
...
func (tds *truncateDatasetSuite) TestToSQL__withPrepared() {
	md := new(mocks.SQLDialect)
	ds := builder.Truncate("test").Prepared(true).SetDialect(md)
	c := ds.GetClauses()
...
func (tds *truncateDatasetSuite) TestExecutor() {
	...
	ds := builder.New("mock", conn).Truncate("table1", "table2")

	tsql, args, err := ds.ToSQL()
	tds.NoError(err)
	tds.Empty(args)
	tds.Equal(`TRUNCATE "table1", "table2"`, tsql)

	tsql, args, err = ds.Prepared(true).ToSQL()
	tds.NoError(err)
	tds.Empty(args)
	tds.Equal(`TRUNCATE "table1", "table2"`, tsql)
