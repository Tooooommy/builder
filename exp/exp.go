...
type (
	...
	Bitwiseable interface {
		...
		// Creates a Bit Operation Expresion for sql |
		// I("col").BitOr(1) // ("col" | 1)
		BitwiseOr(any) BitwiseExpression
		// Creates a Bit Operation Expresion for sql &
		// I("col").BitAnd(1) // ("col" & 1)
		BitwiseAnd(any) BitwiseExpression
		// Creates a Bit Operation Expresion for sql ^
		// I("col").BitXor(1) // ("col" ^ 1)
		BitwiseXor(any) BitwiseExpression
		// Creates a Bit Operation Expresion for sql <<
		// I("col").BitLeftShift(1) // ("col" << 1)
		BitwiseLeftShift(any) BitwiseExpression
		// Creates a Bit Operation Expresion for sql >>
		// I("col").BitRighttShift(1) // ("col" >> 1)
		BitwiseRightShift(any) BitwiseExpression
	}
)
...
type (
	...
	WindowExpression interface {
		...
		PartitionBy(cols ...any) WindowExpression
		OrderBy(cols ...any) WindowExpression
	}
	CaseElse interface {
		Result() any
	}
	...
	CaseExpression interface {
		...
		GetWhens() []CaseWhen
		GetElse() CaseElse
		Value(val any) CaseExpression
		When(condition, result any) CaseExpression
		Else(result any) CaseExpression
	}
)
