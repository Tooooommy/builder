...
func NewInsertExpression(rows ...any) (insertExpression InsertExpression, err error) {
	switch len(rows) {
	case 0:
...
func (i *insert) Vals() [][]any {
	return i.vals
}

func (i *insert) SetVals(vals [][]any) InsertExpression {
	ci := i.clone()
	ci.vals = vals
	return ci
}
...
func newInsert(rows ...any) (insertExp InsertExpression, err error) {
	...
	for _, row := range rows {
		...
		case reflect.Map:
			if columns == nil {
				...
				sort.Sort(mapKeys)
				colKeys := make([]any, 0, len(mapKeys))
			...
			rowVals := make([]any, 0, len(mapKeys))
			for _, key := range mapKeys {
		...
		default:
			return nil, errors.New(
				"unsupported insert must be map, builder.Record, or struct type got: %T",
				row,
			)
		}
	}
...
func createStructSliceInsert(rows ...any) (insertExp InsertExpression, err error) {
	rowValue := reflect.Indirect(reflect.ValueOf(rows[0]))
	rowType := rowValue.Type()
	recordRows := make([]any, 0, len(rows))
	for _, row := range rows {
		if rowType != reflect.Indirect(reflect.ValueOf(row)).Type() {
			return nil, errors.New(
				"rows must be all the same type expected %+v got %+v",
