...
func (tse testSQLExpression) ToSQL() (sql string, args []any, err error) {
	return "", nil, nil
}
...
func (scs *selectClausesSuite) TestCompounds() {
	ce := exp.NewCompoundExpression(exp.UnionCompoundType, newTestAppendableExpression("SELECT * FROM foo", []any{}))
...
func (scs *selectClausesSuite) TestCompoundsAppend() {
	ce := exp.NewCompoundExpression(exp.UnionCompoundType, newTestAppendableExpression("SELECT * FROM foo1", []any{}))
	ce2 := exp.NewCompoundExpression(exp.UnionCompoundType, newTestAppendableExpression("SELECT * FROM foo2", []any{}))
...
func (scs *selectClausesSuite) TestCommonTables() {
	cte := exp.NewCommonTableExpression(true, "test", newTestAppendableExpression(`SELECT * FROM "foo"`, []any{}))
