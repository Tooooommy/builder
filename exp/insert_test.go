...
func newTestAppendableExpression(sql string, args []any) exp.AppendableExpression {
...
func (iets *insertExpressionTestSuite) TestNewInsertExpression_withDifferentRecordTypes() {
	...
	iets.EqualError(err, "builder: rows must be all the same type expected exp_test.testRecord got exp.Record")
	_, err = exp.NewInsertExpression(
		testRecord{C: "v1"},
		testRecord2{C: "v2"},
	)
	iets.EqualError(err, "builder: rows must be all the same type expected exp_test.testRecord got exp_test.testRecord2")
}

func (iets *insertExpressionTestSuite) TestNewInsertExpression_withInvalidValue() {
	_, err := exp.NewInsertExpression(true)
	iets.EqualError(err, "builder: unsupported insert must be map, builder.Record, or struct type got: bool")
}
...
func (iets *insertExpressionTestSuite) TestNewInsertExpression_withDifferentTypes() {
	...
	iets.EqualError(err, "builder: rows must be all the same type expected exp.Record got bool")
}
...
func (iets *insertExpressionTestSuite) TestNewInsertExpression_From() {
	ae := newTestAppendableExpression("select * from test", []any{})
...
func (iets *insertExpressionTestSuite) TestNewInsertExpression_withRecordOfDifferentLength() {
	...
	iets.EqualError(err, "builder: rows with different value length expected 1 got 2")
}
...
func (iets *insertExpressionTestSuite) TestNewInsertExpression_withRecordWithDifferentkeys() {
	...
	iets.EqualError(err, `builder: rows with different keys expected ["c1"] got ["c2"]`)
}
...
func (iets *insertExpressionTestSuite) TestNewInsertExpression_withStructsWithoutTags() {
	...
	iets.Equal([][]any{{int64(1), true, "a"}, {int64(2), false, "b"}}, ie.Vals())
	iets.False(ie.IsEmpty())
	iets.False(ie.IsInsertFrom())
}
...
func (iets *insertExpressionTestSuite) TestNewInsertExpression_withStructsIgnoredDbTag() {
	...
	iets.Equal([][]any{{true, "a"}, {false, "b"}}, ie.Vals())
	iets.False(ie.IsEmpty())
	iets.False(ie.IsInsertFrom())
}

func (iets *insertExpressionTestSuite) TestNewInsertExpression_withStructsWithbuilderSkipInsert() {
	type testRecord struct {
		FieldA int64
		FieldB bool   `builder:"skipupdate"`
		FieldC string `builder:"skipinsert"`
	}
	ie, err := exp.NewInsertExpression(
		testRecord{FieldA: 1, FieldB: true, FieldC: "a"},
		testRecord{FieldA: 2, FieldB: false, FieldC: "b"},
	...
	iets.Equal([][]any{{int64(1), true}, {int64(2), false}}, ie.Vals())
	iets.False(ie.IsEmpty())
	iets.False(ie.IsInsertFrom())
}
...
func (iets *insertExpressionTestSuite) TestNewInsertExpression_withDifferentStructTypes() {
	...
	iets.EqualError(err, "builder: rows must be all the same type expected exp_test.item got exp_test.Phone")
}
...
func (iets *insertExpressionTestSuite) TestNewInsertExpression_withDifferentColumnLengths() {
	...
	iets.EqualError(err, `builder: rows with different keys expected `+
