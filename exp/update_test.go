...
func (uets *updateExpressionTestSuite) TestNewUpdateExpressions_withInvalidValue() {
	...
	uets.EqualError(err, "builder: unsupported update interface type bool")
}
...
func (uets *updateExpressionTestSuite) TestNewUpdateExpressions_withMap() {
	ie, err := exp.NewUpdateExpressions(map[string]any{"c": "a", "b": "d"})
	uets.NoError(err)
	eie := []exp.UpdateExpression{
		exp.NewIdentifierExpression("", "", "b").Set("d"),
		exp.NewIdentifierExpression("", "", "c").Set("a"),
...
func (uets *updateExpressionTestSuite) TestNewUpdateExpressions_withStructsWithbuilderSkipUpdate() {
	type testRecord struct {
		FieldA int64
