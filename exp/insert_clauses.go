...
type (
	InsertClauses interface {
		...
		Rows() []any
		HasRows() bool
		SetRows(rows []any) InsertClauses
		...
		Vals() [][]any
		HasVals() bool
		SetVals(vals [][]any) InsertClauses
		ValsAppend(vals [][]any) InsertClauses
...
func (ic *insertClauses) Rows() []any {
	return ic.rows
}
...
func (ic *insertClauses) SetRows(rows []any) InsertClauses {
	ret := ic.clone()
	ret.rows = rows
	return ret
}

func (ic *insertClauses) Vals() [][]any {
	return ic.values
}
...
func (ic *insertClauses) SetVals(vals [][]any) InsertClauses {
	ret := ic.clone()
	ret.values = vals
	return ret
}

func (ic *insertClauses) ValsAppend(vals [][]any) InsertClauses {
	ret := ic.clone()
	newVals := make([][]any, 0, len(ic.values)+len(vals))
