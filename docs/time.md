...
By default when interpolating `time.Time` (and `*time.Time`) `builder` will convert it `UTC` before interpolating.
...
`builder` provides a **_global_** configuration settings to set the [location](https://golang.org/pkg/time/#Location) to convert all timestamps to. 

To change the default timezone to covert time instances to you can use [`builder.SetTimeLocation`](https://godoc.org/github.com/Tooooommy/builder#SetTimeLocation) to change the default timezone.
