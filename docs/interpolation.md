...
By default the `builder` will interpolate all parameters, if you do not want to have values interpolated you can use the [`Prepared`](http://godoc.org/github.com/Tooooommy/builder#SelectDataset.Prepared) method to prevent this.
...
sql, args, _ := preparedDs.Where(builder.Ex{
	"col1": "a",
	"col2": 1,
	"col3": true,
	"col4": false,
...
```go
var items []Item
sql, args, _ := db.From("items").Prepared(true).Where(builder.Ex{
	"col1": "a",
	"col2": 1,
}).QueryRows(&items)

//Is the same as
db.QueryRows(&items, `SELECT * FROM "items" WHERE (("col1" = ?) AND ("col2" = ?))`,  "a", 1)
```
