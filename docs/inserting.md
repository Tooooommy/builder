...
* Examples
  ...
  * [Insert `builder.Record`](#insert-record)
...
<a name="create"></a>
To create a [`InsertDataset`](https://godoc.org/github.com/Tooooommy/builder/#InsertDataset)  you can use

**[`builder.Insert`](https://godoc.org/github.com/Tooooommy/builder/#Insert)**
...
**[`SelectDataset.Insert`](https://godoc.org/github.com/Tooooommy/builder/#SelectDataset.Insert)**
...
**[`DialectWrapper.Insert`](https://godoc.org/github.com/Tooooommy/builder/#DialectWrapper.Insert)**
...
**[`Database.Insert`](https://godoc.org/github.com/Tooooommy/builder/#DialectWrapper.Insert)**
...
For more examples visit the **[Docs](https://godoc.org/github.com/Tooooommy/builder/#InsertDataset)**
...
```go
type User struct {
	FirstName string `db:"first_name" builder:"skipinsert"`
	LastName  string `db:"last_name"`
}
ds := builder.Insert("user").Rows(
	User{FirstName: "Greg", LastName: "Farley"},
...
```go
type User struct {
	FirstName string `db:"first_name" builder:"defaultifempty"`
	LastName  string `db:"last_name"`
}
ds := builder.Insert("user").Rows(
...
`builder` will also use fields in embedded structs when creating an insert.
...
<a name="seterror"></a>
**[`SetError`](https://godoc.org/github.com/Tooooommy/builder/#InsertDataset.SetError)**
...
```go
func GetInsert(name string, value string) *builder.InsertDataset {

    var ds = builder.Insert("test")
    ...
    return ds.Rows(builder.Record{name: value})
}
...
To execute INSERTS use [`Database.Insert`](https://godoc.org/github.com/Tooooommy/builder/#Database.Insert) to create your dataset
...
insert := db.Insert("builder_user").Rows(users).Executor()
if _, err := insert.Exec(); err != nil {
	fmt.Println(err.Error())
} else {
...
insert := db.Insert("builder_user").Returning(builder.C("id")).Rows(
		builder.Record{"first_name": "Jed", "last_name": "Riley", "created": time.Now()},
).Executor()

var id int64
if _, err := insert.QueryRow(&id); err != nil {
	fmt.Println(err.Error())
} else {
