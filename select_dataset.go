...
func newDataset(d string, executor sqlx.Session) *SelectDataset {
...
func (sd *SelectDataset) Update() *UpdateDataset {
	u := newUpdateDataset(sd.dialect.Dialect(), sd.executor).
...
func (sd *SelectDataset) Insert() *InsertDataset {
	i := newInsertDataset(sd.dialect.Dialect(), sd.executor).
		Prepared(sd.isPrepared.Bool())
	if sd.clauses.HasSources() {
...
func (sd *SelectDataset) Delete() *DeleteDataset {
	d := newDeleteDataset(sd.dialect.Dialect(), sd.executor).
		Prepared(sd.isPrepared.Bool())
	if sd.clauses.HasSources() {
...
func (sd *SelectDataset) Truncate() *TruncateDataset {
	td := newTruncateDataset(sd.dialect.Dialect(), sd.executor)
	if sd.clauses.HasSources() {
...
// func (sd *SelectDataset) Executor() {
// 	return sd.executor.FromSQLBuilder(sd.selectSQLBuilder())
// }
...
//
// QueryRow will only select the columns that can be scanned in to the struct unless you have explicitly selected
// certain columns. See examples.
//
// i: A pointer to a structs
func (sd *SelectDataset) QueryRow(v any) error {
	return sd.QueryRowCtx(context.Background(), v)
}

// Generates the SELECT sql for this dataset and uses Exec#QueryRowContext to scan the result into a slice of structs
//
// QueryRowContext will only select the columns that can be scanned in to the struct unless you have explicitly
// selected certain columns. See examples.
//
// i: A pointer to a structs
func (sd *SelectDataset) QueryRowCtx(ctx context.Context, v any) error {
	if sd.executor == nil {
		return ErrExecutorNotFoundError
	}
	ds := sd
	if sd.GetClauses().IsDefaultSelect() {
		ds = sd.Select(v)
	}
	query, args, err := sd.Limit(1).selectSQLBuilder().ToSQL()
	if err != nil {
		return err
	}
	return ds.executor.QueryRowCtx(ctx, v, query, args...)
}
...
func (sd *SelectDataset) QueryRowPartial(v any) error {
	return sd.QueryRowPartialCtx(context.Background(), v)
}
...
func (sd *SelectDataset) QueryRowPartialCtx(ctx context.Context, v any) error {
	...
	return ds.executor.QueryRowPartialCtx(ctx, v, query, args...)
}

// Generates the SELECT sql for this dataset and uses Exec#QueryRows to scan the results into a slice of structs.
//
// QueryRows will only select the columns that can be scanned in to the struct unless you have explicitly selected
// certain columns. See examples.
//
// i: A pointer to a slice of structs
func (sd *SelectDataset) QueryRows(v any) error {
	return sd.QueryRowsCtx(context.Background(), v)
}

// Generates the SELECT sql for this dataset and uses Exec#QueryRowsContext to scan the results into a slice of
// structs.
//
// QueryRowsContext will only select the columns that can be scanned in to the struct unless you have explicitly
// selected certain columns. See examples.
//
// i: A pointer to a slice of structs
func (sd *SelectDataset) QueryRowsCtx(ctx context.Context, v any) error {
	...
	query, args, err := sd.selectSQLBuilder().ToSQL()
	if err != nil {
		return err
	}
	return ds.executor.QueryRowsCtx(ctx, v, query, args...)
}
...
func (sd *SelectDataset) QueryRowsPartial(v any) error {
	return sd.QueryRowsPartialCtx(context.Background(), v)
}
...
func (sd *SelectDataset) QueryRowsPartialCtx(ctx context.Context, v any) error {
	...
	return ds.executor.QueryRowsPartialCtx(ctx, v, query, args...)
}
...
func (sd *SelectDataset) CountContext(ctx context.Context) (int64, error) {
	var count int64
	query, args, err := sd.Select(COUNT(Star()).As("count")).ToSQL()
	if err != nil {
		return count, err
	}
	err = sd.executor.QueryRowCtx(ctx, &count, query, args...)
	return count, err
}

// Generates the SELECT sql only selecting the passed in column and uses Exec#QueryRows to scan the result into a slice
// of primitive values.
//
...
func (sd *SelectDataset) Pluck(v any, col string) error {
	return sd.PluckContext(context.Background(), v, col)
}

// Generates the SELECT sql only selecting the passed in column and uses Exec#QueryRowsContext to scan the result into a
...
func (sd *SelectDataset) PluckContext(ctx context.Context, v any, col string) error {
	...
	query, args, err := sd.Select(col).selectSQLBuilder().ToSQL()
	if err != nil {
		return err
	}
	return sd.executor.QueryRowsPartialCtx(ctx, v, query, args...)
}
